"""
Submit test job to Databricks serverless — pin numpy for sklearn compat.
"""
...
# Key insight: Databricks has numpy 2.x installed which broke older sklearn.
# Force upgrade sklearn to >=1.5.0 which is numpy 2.x compatible.
NOTEBOOK_CONTENT = r'''# Databricks notebook source
# MAGIC %pip install "numpy>=1.24,<2.0" "pandas>=2.0" "scipy>=1.12" "statsmodels>=0.14" "scikit-learn>=1.5" pytest --quiet
...
nb_path = "/Workspace/insurance-glm-cluster/notebooks/test_runner_v4"
...
try:
    ...
    if content.content:
        ...
        print("\n=== FULL TEST OUTPUT ===")
        print(decoded[-8000:] if len(decoded) > 8000 else decoded)
